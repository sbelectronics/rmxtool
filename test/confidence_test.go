...
func (s *ConfidenceSuite) TestDeletePutContig() {
	files := make(map[string]string)
	for k, v := range SRCIMAGE_FILES {
		files[k] = v
	}
	...
	// odyssey.txt will fail because even though diskverify is larger size, diskverify has multiple ranges
	out, errOut, err = s.run("put", "-q", "testdata/odyssey.txt", "-f", TESTIMAGE, "-d", "/user/world", "--contig")
	s.Error(err, "Expected an error when using the --contig flag with incompatible conditions")

	out, errOut, err = s.run("put", "-q", "testdata/country.txt", "-f", TESTIMAGE, "--contig")
	s.NoError(err)
	s.ShowIfError(err, out, errOut)

	out, errOut, err = s.run("put", "-q", "testdata/lamb.txt", "-f", TESTIMAGE, "-d", "/system", "--contig")
	s.NoError(err)
	s.ShowIfError(err, out, errOut)

	out, errOut, err = s.run("put", "-q", "testdata/scott.txt", "-f", TESTIMAGE, "-d", "/lang", "--contig")
	s.NoError(err)
	s.ShowIfError(err, out, errOut)
